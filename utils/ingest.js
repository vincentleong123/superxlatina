// utils/ingest.js
// Feed system for Super X Latina — ported from wps-transformer-player.
// Turn a JSON array or a plain TXT list of URLs / <iframe> embeds into
// permanent library entries in data/videos.json.
//
...
function makeDescription(title) {
  ...
  return `<h2>${safe}</h2>\n<p>Watch <strong>${safe}</strong> free in HD on Super X Latina.</p>`;
}
...
async function enrich(entry, timeoutMs) {
  ...
  if (source.type === 'embed' && !out.title) {
    try {
      ...
      // Browser-engine fallback: if the plain fetch was empty/js-challenged
      // (e.g. newsexwap.com), render the page to still grab title/thumbnail.
      if (!html || !html.length || !meta.title) {
        try {
          const resolvers = require('./resolvers');
          const r = await resolvers.resolveWithMeta(base, { allowBrowser: true });
          if (r && r.meta) {
            if (r.meta.title) meta.title = r.meta.title;
            if (!meta.thumbnail && r.meta.thumbnail) meta.thumbnail = r.meta.thumbnail;
            if (!meta.duration && r.meta.duration) meta.duration = r.meta.duration;
            if ((!meta.tags || !meta.tags.length) && r.meta.tags) meta.tags = r.meta.tags;
          }
        } catch (e) {}
      }
  ...
  // NOTE: durations are NOT probed here on purpose. Each probe is a ranged media
  // request (doubles the per-URL load during a bulk drop and trips bot flags on
  // tube sites). Durations self-heal later via /api/backfill-durations, which
  // runs slowly in the background — matching the wps-transformer-player behavior.
