// utils/thumb.js
// Cached thumbnail store — ported from wps-transformer-player.
// Download each origin image once (through the transformer's cookie/throttle
// layer), then serve from disk forever so the gallery never hits the origin
// CDN while scrolling, and no repeated hotlinks can ever get us flagged.
'use strict';
...
async function serve(v, res) {
  ...
  return downloadToCache(v).then(({ buf, ct, ok }) => {
...
// Fetch the origin image and persist it to the disk cache (no response needed).
// Returns { ok, buf, ct } so callers can backfill thumbnails outside a request.
async function downloadToCache(v) {
  if (!v || !v.thumbnail) return { ok: false, buf: null, ct: '' };
  let up;
  try {
    up = await transformer.fetchImage({ url: v.thumbnail, referer: '', type: 'direct' });
  } catch (e) {
    return { ok: false, buf: null, ct: '' };
  }
  if (up.statusCode < 200 || up.statusCode >= 300) { up.resume(); return { ok: false, buf: null, ct: '' }; }
  ...
  if (!buf.length) return { ok: false, buf: null, ct: '' };
...
module.exports = { init, fileFor, getDir, serve, downloadToCache };
